    ...
    for filename in logFiles:
        ...
        for line in lines:
            if keywords[0] in line:
                ll = line.strip().split()
                dof.append(int(ll[-1])*appCtx.dof)
                ...
                ll = line.strip().split()
                L2err.append(float("{:.7e}".format(float(ll[-1]))))
            elif keywords[5] in line:
                ll = line.strip().split()
                total_time.append(float("{:.4e}".format(float(ll[2]))))
...
if __name__ == "__main__":
    ...
    df_col_names= ['nu', 'p', 'h', 'DoF', '#CG', 'Solve Time(s)', 'MDoFs/Sec', 'L2 Error', 'Total Time(sec)','np']
